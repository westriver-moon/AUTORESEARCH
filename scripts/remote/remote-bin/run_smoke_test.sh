...
command -v timeout >/dev/null 2>&1 || die "timeout is required for bounded smoke tests"
GPU="$(resolve_gpu "${GPU}")"
...
if [[ "${exit_code}" -eq 0 ]]; then
  ...
  write_status "${STATUS_FILE}" "timeout" "smoke test exceeded max seconds; see log_file" "${exit_code}"
else
