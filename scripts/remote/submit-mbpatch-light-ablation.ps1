param(
    ...
    [string] $ExperimentId,
    [ValidateSet("inspect", "apply", "smoke", "start", "status", "collect", "summarize", "audit")]
    [string] $Mode = "inspect",
...
$entry = Join-RemotePath -Left ([string] $config.RemoteWorkspaceRoot) -Right "bin/run_mbpatch_light_ablation_bridge.sh"
Assert-RemotePath -Path $entry -Name "MbpatchLightEntry"
...
$status = New-StatusObject -ScriptName "submit-mbpatch-light-ablation.ps1" -Ok $ok -ExperimentId $ExperimentId -Details @{
    mode = $Mode
    remote_entry = $entry
...
$outPath = Get-StatusFilePath -ProjectRoot $projectRoot -ExperimentId $ExperimentId -Name ("mbpatch-light-" + $Mode)
