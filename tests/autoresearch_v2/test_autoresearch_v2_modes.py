...
class AutoresearchV2ModePolicyTest(unittest.TestCase):
    ...
    def test_policy_is_the_only_complete_sealed_path_list(self) -> None:
        invoke_text = INVOKE_SKILL.read_text(encoding="utf-8")
        dev_text = DEV_SKILL.read_text(encoding="utf-8")

        self.assertIn(".codex/research-policy.json", dev_text)
        self.assertNotIn("scripts/remote/access/**", dev_text)
        self.assertNotIn("scripts/remote/access/**", invoke_text)
