...
Project-local adapter override: foreground mode only. Do not use background
runtime control, `codex exec`, hooks, Full Access, or bypass flags.
...
- `Verify format` 鈥?`scalar` (default) or `metrics_json`; use `metrics_json` when the verify command outputs a JSON object with multiple metrics as its final line
- `Primary metric key` 鈥?which key in the metrics JSON to use as the TSV primary metric; defaults to the metric name
- `Acceptance criteria` 鈥?list of `{metric_key, operator, target}` thresholds the retained result must satisfy before the run can stop (see `references/results-logging.md` Metrics And Acceptance Contract)
- `Required keep criteria` 鈥?list of `{metric_key, operator, target}` hard gates every retained result must satisfy to enter `keep` state; use when some metrics must never regress regardless of primary metric improvement
...
```bash
python <skill-root>/scripts/autoresearch_launch_gate.py --repo /path/to/repo
```
...
Exec mode is disabled in this project-local adapter.
...
```bash
python <skill-root>/scripts/autoresearch_init_run.py --repo <primary_repo> --workspace-root <workspace_root> ...
```

This writes the baseline TSV row (`iteration = 0`) and the matching JSON snapshot in one step.

Here `<skill-root>` is the directory containing the loaded `SKILL.md`. In the common repo-local install this is usually `.agents/skills/codex-autoresearch`, so the exact command becomes `python .agents/skills/codex-autoresearch/scripts/...`.
...
Do not initialize exec scratch state in this project-local adapter.
...
**Two-phase boundary:** All questions happen BEFORE launch. This project-local
adapter always launches in **foreground** mode after approval. Keep the loop in
the same Codex session and use the shared helper scripts directly. Do not call
`autoresearch_runtime_ctl.py`, do not create background launch/runtime files,
and do not invoke `codex exec`. After launch, NEVER pause to ask the user
anything during the loop -- not for clarification, not for confirmation, not for
permission. If you encounter ambiguity mid-loop, apply best practices, log your
reasoning in the commit message, and keep iterating.
...
- Do not use exec-mode dirty-worktree behavior in this adapter; exec is disabled.
...
- For serial/main rows, prefer:
  ```bash
  python <skill-root>/scripts/autoresearch_record_iteration.py ...
  ```
- For parallel batches, prefer:
  ```bash
  python <skill-root>/scripts/autoresearch_select_parallel_batch.py --batch-file ...
  ```
...
After every `keep` decision, `autoresearch_record_iteration.py` appends a
positive lesson immediately after the authoritative TSV/JSON update. After
every PIVOT, the same helper appends a strategic lesson the same way. See
`references/lessons-protocol.md` for structure and persistence.
...
- **Foreground integrity checks:** use `autoresearch_health_check.py` for disk space, git state, verify command existence, and resume-helper-based TSV/JSON integrity when a recovery or integrity concern appears.
- **Commit safety:** when managed repos are git-backed, use `autoresearch_commit_gate.py` or equivalent foreground checks before committing trial changes. Continue only if staged autoresearch artifacts and out-of-scope worktree changes are absent in every managed repo.
