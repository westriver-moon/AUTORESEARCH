...
8. The mandatory confirmation round must never collapse into a bare "go" prompt. Always show a short repo-grounded summary of the confirmed goal, metric, verify path, and next step before asking for launch approval.
...
10. After the user approves the summary, launch foreground mode directly from the same skill entrypoint. Do not persist a launch manifest, start a runtime controller, or tell the user to switch to a different wrapper command.
...
12. Hooks are disabled in this project-local adapter. Do not check, install, or repair hooks during the wizard.
...
Do not check or install managed hooks after the scan. This adapter is foreground-only and hook-free.
...
**Next step**
- Reply "go" to start in foreground mode, or tell me what to change.
```
...
6. Do not ask the user to choose background or exec. This project-local adapter always uses foreground mode.
...
10. Never show a hooks install note. Hooks are disabled in this project-local adapter.
...
1. Use foreground mode only.
2. Do not check, install, repair, or enable hooks.
3. Keep the loop in the current Codex session:
...
4. Do not ask the user to rerun a shell wrapper command just to continue.
5. If recovery analysis requires a fresh start, archive old foreground artifacts with `.prev` suffixes through the foreground helper flow, not through the background runtime controller.
...
- "Want me to run 10 iterations as a test, or keep iterating in this foreground session until interrupted?"
- "Should this be an ongoing foreground run until you interrupt it, or a bounded trial run?"
...
Exec mode is disabled in this project-local adapter. If the user asks for it, decline that mode and offer a foreground run.

### Execution Policy

- Do not create background launch manifests.
- Do not set or rely on `danger_full_access`.
- Do not use `--dangerously-bypass-approvals-and-sandbox` or any Full Access bypass path.
...
4. The user replies "go" and the loop starts immediately in foreground mode:
