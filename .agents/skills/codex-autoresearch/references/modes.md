...
Official Codex activation in this project is explicit only:
`$codex-autoresearch` with an optional supported `Mode: <name>`.
...
4. use `interaction-wizard.md` for every new interactive launch,
...
8. write their documented logs and output files (for foreground iterating modes this means `autoresearch-results/results.tsv` and `autoresearch-results/state.json` as the core persistent artifacts; lessons remain helper-derived secondary output),
9. preserve the official skill entrypoint in `SKILL.md`.

`Mode: exec`, background runtime modes, hooks, and Full Access bypass paths are
disabled in this project-local adapter.
