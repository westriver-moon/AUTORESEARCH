...
Background control files (`launch.json`, `runtime.json`, and `runtime.log`) are
disabled in this project-local foreground-only adapter.
...
Foreground resume resolves context in this order: current repo pointer,
canonical `autoresearch-results/context.json`, then fail with a clear error. Do
not walk upward from cwd looking for guessed contexts, and do not infer repo
identity from a results path.
...
`config.session_mode` is the authoritative interactive-mode marker. In this
adapter it must be `foreground`. Foreground runs resume from `results.tsv` plus
`state.json`.

Do not switch an existing interactive run into background mode. If stale state
claims `background`, repair or fresh-start into foreground only.
...
The `supervisor` object is optional. It is not required for normal foreground
session resume and should be preserved if present.
...
```bash
python <skill-root>/scripts/autoresearch_resume_check.py --repo /path/to/repo
```
...
- any foreground resume prompt generation
...
7. Legacy repo-root artifacts are not restored into the new schema; use a foreground fresh start instead of synthesizing compatibility artifacts.
...
4. If starting fresh, archive prior persistent foreground artifacts with `.prev` suffixes and proceed with the full wizard. Do not use the background runtime controller.
...
2. If the user wants to resume, prefer:
   ```bash
   python <skill-root>/scripts/autoresearch_resume_check.py --repo /path/to/repo --write-repaired-state
   ```
3. Present one condensed confirmation block sourced from the reconstructed state.
4. After confirmation, continue from the next main iteration in foreground mode. Foreground runs resume directly from results/state.
5. Do not start a detached runtime from TSV fallback.
...
2. Rename prior persistent foreground artifacts in `autoresearch-results/` to `.prev` variants if they exist.
...
If the recovered config clearly belongs to a different goal than the current
request, start fresh and archive the old foreground artifacts to `.prev` through
the foreground helper flow.
...
- **exec-workflow.md:** Exec mode is disabled in this project-local adapter.
