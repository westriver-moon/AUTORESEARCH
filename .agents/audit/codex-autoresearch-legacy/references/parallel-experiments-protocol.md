...
- Prefer `python <skill-root>/scripts/autoresearch_select_parallel_batch.py --batch-file ...` so worker rows, the authoritative main row, batch-boundary preflight checks, JSON state, and any resulting keep lesson stay aligned. Here `<skill-root>` is the directory containing the loaded `SKILL.md`.
