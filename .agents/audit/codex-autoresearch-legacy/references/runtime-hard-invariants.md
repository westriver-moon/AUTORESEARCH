...
8. All normal run artifacts are workspace-owned under `autoresearch-results/`: `results.tsv`, `state.json`, `context.json`, and `lessons.md`; do not create background control artifacts (`launch.json`, `runtime.json`, or `runtime.log`) in this project-local adapter.
