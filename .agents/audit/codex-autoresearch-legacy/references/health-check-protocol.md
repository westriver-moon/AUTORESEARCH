...
Self-monitoring system that validates environment and foreground run integrity.
Catches problems before they corrupt results.

The executable companions are:

- `python <skill-root>/scripts/autoresearch_health_check.py --repo <primary_repo>`
- `python <skill-root>/scripts/autoresearch_commit_gate.py`
...
### Foreground Integrity Check (Lightweight)

Run before a fresh foreground launch, before a sensitive recovery decision, or
when runtime integrity looks suspect:
...
| Log integrity | `python <skill-root>/scripts/autoresearch_resume_check.py --repo <repo>` can reconstruct TSV state | Hard blocker if the TSV is corrupt |
...
- **autonomous-loop-protocol.md:** Runs as the detailed reference for health checks and re-anchoring. Context health feeds into the Protocol Fingerprint Check defined in `runtime-hard-invariants.md`.
...
- **multi-repo runs:** the helper remains anchored in the primary repo for results/state/log integrity, but companion repos participate in worktree-scope checks through the foreground repo list.
